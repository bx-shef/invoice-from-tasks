...
| `tasks.task.result.list` | фрейм, **v3** | `useInvoiceFill.ts` | Отчёты задачи — контекст названий в режиме BitrixGPT (тип 1). Метод REST v3: условие `['taskId', '=', id]` в `filter` обязательно, `pagination.limit`, ответ — `text` у каждого результата. Сбой не останавливает заполнение. | 📄 |
